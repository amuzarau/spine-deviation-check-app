...
> ⚠️ Приложение **НЕ является медицинским диагностическим инструментом**.  
> Оно предназначено для **оценки потенциального риска** (низкий / средний / высокий) и **принятия решения о необходимости дальнейшего медицинского обследования**.
...
🔄 Flow работы приложения (Sequence Diagram)
sequenceDiagram
...
Что используется из MediaPipe

Pose — детекция 33 ключевых точек тела

landmarks плеч, таза, головы, голеностопа
...
асимметрия плеч (LEFT_SHOULDER vs RIGHT_SHOULDER)

асимметрия таза (LEFT_HIP vs RIGHT_HIP)
...
⚠️ Ограничение MVP
Используется координатная разница, а не угловая модель, что приводит к:

высокой чувствительности к положению камеры

консервативной (предупредительной) оценке риска

⚠️ Почему точность сейчас ограничена (осознанно)

Это MVP без нейросетей, поэтому:

не измеряются реальные углы кифоза/лордоза;

не анализируется линия позвоночника;

отсутствует персональная калибровка камеры;

результат намеренно консервативный (минимизация false negative).

🚀 Пути развития проекта
🔹 Направление 1: Без нейросетей (Computer Vision)
...
более дифференцированные результаты

снижение ложных срабатываний

🔹 Направление 2: С нейросетями (ML / DL)

Возможности:

CNN / Vision Transformers

обучающая выборка (размеченные фото)
...
адаптация под реальные медицинские кейсы

🧩 Возможные дополнительные фичи

📧 отправка фото и контактов родителя врачу-ортопеду (EmailJS)
...
⭐ Если вы врач, педагог или разработчик — обратная связь приветствуется! Свяжитесь со мной через форму в разделе Контакты моего веб-сайта https://andreimuzarau.com/
